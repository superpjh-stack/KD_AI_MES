...
# 회전 22 — G-08 디지털 스레드를 합성 데이터로 채웠다 (사용자 지시 · D-131 · D-139)

> **이 회전의 수치는 전부 `합성 데이터 기준 (D-131)` 이다.** 성과 실적으로 인용할 수 없다.
> 실측·확정·가설·합성을 아래처럼 **갈라서** 적는다 — 섞어 한 수치로 보고하지 않는다.

## ① 코드 마스터 — 확정 19종 · 가설 179종

| 구분 | 종수 | 근거 | 표시가 남는 자리 |
|---|---|---|---|
| **고객사 — 확정 (D-139)** | **19** (발주처 12 · 수요처 7) | `docs/cad/customer_candidates.json` 의 `후보` 중 `분류` 가 발주처 후보·수요처(엔드유저) 후보인 것. 사용자 확정 2026-09-12 | `ATTR1` = `<역할> · 확정 (D-139) — 사용자 확정 2026-09-12 · 관측 기반` · `ATTR2` = 확신도 |
| **품목 — 가설 (D-131)** | **179** | 과거 견적서 `.xls` **고유 문서 26건** 중 **19문서의 품목명세 462행**(D-118-b 와 일치). 정규화·중복 제거 뒤 179종 | `ATTR1` = `가설 (D-131) — 과거 견적서 품목명세 실측(D-118-b). 도입기업 코드체계 미확정` · `ATTR2` = `견적 N행 · 문서 N건` |

**확신도 분포** 높음 8 · 중간 6 · 낮음 5.
**넣지 않은 것** 협력사·공급사 16(VEN) · 관계사 1(REL-001 케이테크 — D-137) · 타사 도면 소유사 1 ·
도입기업 본인 1(SELF) · 역할 미정 8(UNK) · 사람이름 17 · 제품명 26 · 출처 의심 36.
**재질·보관위치·불량유형·클레임유형·원가대상 5그룹은 그대로 0건이다**(D-47) — 사용자가 확정한 것은
고객사·품목 둘뿐이다. 그래서 `INV_MATERIAL_LOTS.MATERIAL` · `EST_BOM_ITEMS.MATERIAL` ·
`INV_STOCKS.LOCATION_CODE` · `PRC_PERFORMANCES.DEFECT_TYPE` 은 **NULL** 이다.

**품목 추출에서 뺀 것** 집계·원가계정 행 **86행(서로 다른 표기 20종)** — `소 계` · `SUB TOTAL` ·
`合 計` · `자 재 비` · `인 건 비` · `가 공 비` · `연 마 비` · `이 윤` · `OVERHEAD PROFIT` ·
`일반 관리비` 등. **품목이 아니라 집계 행이다.** 뺀 목록 전체가
`docs/cad/quote_items.json` 의 `제외목록` 에 남아 있다. **품목명을 지어내지 않았다** —
그 파일의 `items` 밖에 있는 이름은 시드가 만들지 않고, 테스트가 그것을 단언한다
(`tests/test_dev1_seed.py::test_품목_코드는_가설_표시와_견적_출처를_달고_있다`).

## ② 프로젝트 — **실측 12건** (지어낸 코드 0건)

파싱된 GD 프로젝트 폴더 **99건** 중 **고객사·제품군·도면 셋 다 이을 수 있는 12건**만 썼다.
셋 중 하나라도 못 이으면 **뺐다** — 고객사나 제품군을 지어내 붙이지 않았다.

| 프로젝트번호 | 제품군 | 발주처 | 도면(사본 제거) | 관측된 수요처 |
|---|---|---|---|---|
| GD170402 | PG50 저장탱크 | CUST-007 두루텍 | 18 | — |
| GD170601 / GD170602 | PG50 저장탱크 | CUST-004 웰이엔씨 | 2 / 2 | — |
| GD1802-01 | PG30 진공건조기 | CUST-004 | 42 | — |
| GD1802-02 | PG20 교반기 | CUST-004 | 1 | — |
| GD1904-00-01 | PG50 저장탱크 | CUST-004 | 31 | — |
| GD1904-00-02 | PG20 교반기 | CUST-004 | 3 | — |
| GD1906 | PG50 저장탱크 | CUST-009 대한열기 | 79 | — |
| GD2003-02 | PG40 누체필터 | CUST-004 | 28 | CUST-014 에니젠 |
| GD2003-03 | PG40 누체필터 | CUST-004 | 39 | CUST-013 비나텍 |
| GD2003-06 | PG10 반응기 | CUST-004 | 33 | — |
| GD2005-04-00 | PG20 교반기 | CUST-005 한모루 | 63 | — |
| **합계** | **제품군 5종 전부** | **발주처 4종** | **341** | 2건 |

**실측인 것** 프로젝트번호 · 제품군 · 발주처(조상 폴더명) · 도면 파일명·상대경로·크기·최종수정일.
**실측이 아닌 것** 도면번호(표제란 도면번호는 1,136건 중 434건에만 있다 → **개발1 채번 규칙
`DWG-{제품군3}-NNNN`** 을 붙였다) · 수주일시(프로젝트번호의 **YYMM 에서 파생** — `date.today()` 를
쓰지 않는다, §10-3) · `ORDER_AMOUNT`(비웠다 — 견적 문서와 프로젝트의 1:1 근거가 없다).
선정 규칙은 `docs/cad/thread_projects.json` 의 `_meta.선정규칙` 에 적혀 있다.

**수요처를 버리지 않았다** — `EST_PROJECTS` 에 수요처 칸이 없어(컬럼 추가 금지) JSON 의
`enduser_codes` 에만 남겼다. D-134 의 2단 구조가 실재한다는 관측이다.

## ③ 합성 — 고리별 행 수 (원천이 없다 · D-131)

| 고리 | 표 | 행 | 합성 표시를 어디에 남겼나 |
|---|---|---|---|
| 3 BOM | `EST_BOM_HEADERS` | **40** | **비고 칸이 없다** → 하위 `EST_BOM_ROUTINGS.REMARK` 가 진다 |
| 3 BOM 자재구성 | `EST_BOM_ITEMS` | **240** | `SPEC_TEXT` = `합성 (D-131) — BOM 구성은 견적 품목명을 배분한 것이다…` |
| 3 BOM 공정구조 | `EST_BOM_ROUTINGS` | **280** | `REMARK` = `합성 (D-131) — 사업계획서 1.3 의 10공정 중 제조 7공정…` |
| 4 자재LOT | `INV_MATERIAL_LOTS` | **40** | `SPEC_TEXT` = `합성 (D-131) · 투입 BOM <번호> — 두께·길이·중량·재질은…` |
| (입고) | `INV_RECEIPTS` | **40** | **비고 칸이 없다** |
| (재고) | `INV_STOCKS` | **40** | **비고 칸이 없다** |
| (품질기준 골격) | `BAS_QUALITY_STANDARDS` | **15** (제품군 5 × 검사 3) | `JUDGE_RULE` = `합성 (D-131) — … 기준 수치는 도입기업 제공 대기(TD3-030)` |
| (공급처) | `INV_SUPPLIERS` | **3** | **상호 자체**가 `합성 공급처 01 (D-131)` — 실존 상호를 지어내지 않았다(D-101) |
| 5 작업지시 | `PRC_WORK_ORDERS` | **280** (제품LOT 40 × 제조 7공정) | **비고 칸이 없다** |
| 6 공정실적 | `PRC_PERFORMANCES` | **195** | **비고 칸이 없다** (외주 2공정은 실적을 만들지 않는다 — D-41) |
| (공정이력) | `PRC_PROCESS_HISTORIES` | **280** | **비고 칸이 없다** |
| 7 제품LOT | `SHP_LOT_TRACES` | **40** | **비고 칸이 없다** |
| 8 검사 | `SHP_INSPECTIONS` | **117** | **비고 칸이 없다** (`REJECT_REASON` 은 불합격 사유 칸이다) |
| 9 출하 | `SHP_SHIPMENTS` | **39** | **비고 칸이 없다** |
| 9 출하품목 | `SHP_SHIPMENT_ITEMS` | **39** | `REMARK` = `합성 (D-131) — 출하 이후 전 구간이 합성이다…` |
| KPI 실적 | `KPI_MEASURES` | **22** | `REMARK` = `합성 데이터 기준 (D-131) — 표본 N건. … 성과 실적이 아니다` |

**⚠ 합성 표시를 담을 비고/REMARK 계열 컬럼이 없는 표가 9종이다** — 컬럼을 추가하지 않았다
(G-02 762 고정). 목록·근거는 **D-131-보고** 에 있고 `db/seed_dev1.py` 실행 출력이 매번 찍는다.
그 9표의 표시는 ① 게이트 판정 줄 ② **화면 배지** ③ `SYS_CONFIGS('시스템설정','SYNTHETIC_THREAD')`
선언이 나른다. 선언 한 행을 지우면 배지와 게이트 고지가 **함께** 사라지므로 데이터만 남기고
표시만 떼는 일이 불가능하다.

**화면 배지** `templating.render()` 가 선언을 읽어 `base.html` 머리에
`합성 데이터 기준 (D-131)` 배지를, 바닥에 "자재LOT·작업지시·공정실적·LOT추적·검사·출하는 원천이
없는 합성이다 … 이 화면의 수치를 성과 실적으로 인용할 수 없다" 를 렌더한다 — **50화면 전부**다.

## ④ 의도적 단절 — **5 / 40 = 12.5%** (위치를 고정했다)

제품LOT 전역 순번(**프로젝트번호·BOM번호 오름차순**, 0부터)에 `db/seed_dev1.BREAKS` 로 박았다.
무작위가 아니고 시드를 다시 돌려도 같은 자리다.

| 순번 | 프로젝트 / BOM | 무엇을 끊었나 | 끊기는 사슬 단계 |
|---|---|---|---|
| **#3** | GD170402 / BOM-GD170402-04 | `SHP_LOT_TRACES.BOM_ID` 를 비웠다 | 3 BOM |
| **#9** | GD1802-01 / BOM-GD1802-01-02 | `EST_BOM_HEADERS.DRAWING_ID` 를 비웠다 | 2 도면 |
| **#16** | GD1904-00-01 / BOM-GD1904-00-01-04 | `SHP_LOT_TRACES.MATERIAL_LOT_ID` 를 비웠다 | 4 자재LOT |
| **#24** | GD2003-02 / BOM-GD2003-02-01 | `PRC_PERFORMANCES` 를 만들지 않았다 | 6 공정실적 |
| **#33** | GD2003-06 / BOM-GD2003-06-02 | `SHP_INSPECTIONS`·`SHP_SHIPMENTS` 를 만들지 않았다 | 8 검사 · 9 출하 |

**단절을 빼서 100% 를 만들지 않았다.** G-08 이 이 5건을 전부 잡아낸다 —
독립 재계산 35/40 과 선언값 `MAPPING_OK_YN='Y'` 35건이 **정확히 일치**한다(D-213).

## ⑤ G-08 실측 수치와 판정 — **합성 데이터 기준 (D-131)**

```
G-08   PASS   합성 데이터 기준 D-131 · 표본 40 · LOT 매핑 87.5 % (기준 85.0%)
              · 사슬 끊김 정방향 0 · 역방향 0
```

| 사슬 단계 | 정방향 | 역방향 |
|---|---|---|
| 1 `EST_PROJECTS` → 2 `EST_CAD_DRAWINGS` | 12/12 (100.0%) | 341/341 (100.0%) |
| 2 → 3 `EST_BOM_HEADERS` | 39/341 (11.4%) | 39/40 (97.5%) |
| 3 → 4 `INV_MATERIAL_LOTS` (`SHP_LOT_TRACES` 경유) | 38/40 (95.0%) | 38/40 (95.0%) |
| 4 → 5 `PRC_WORK_ORDERS` (경유) | 39/40 (97.5%) | 39/280 (13.9%) |
| 5 → 6 `PRC_PERFORMANCES` | 195/280 (69.6%) | 195/195 (100.0%) |
| 6 → 7 `SHP_LOT_TRACES` | 195/195 (100.0%) | 39/40 (97.5%) |
| 7 → 8 `SHP_INSPECTIONS` | 39/40 (97.5%) | 117/117 (100.0%) |
| 8 → 9 `SHP_SHIPMENTS` (`SHP_SHIPMENT_ITEMS` 경유) | 39/117 (33.3%) | 39/39 (100.0%) |
| **LOT 매핑 (독립 재계산)** | **9단계 완주 35 / 전체 40 → 87.5 %** | 선언값도 35 → 87.5 % (일치) |

**임계 85% 를 넘었다 (87.5%).** 못 넘었으면 못 넘은 대로 적었다 —
임계값·판정 로직은 한 글자도 건드리지 않았고 `check_data.py` 에는 **고지만** 붙였다.
낮은 비율(11.4% · 13.9% · 33.3% · 69.6%)은 **끊김이 아니라 분모 차이**다 —
도면 341건 중 BOM 을 단 것이 39건, 작업지시 280건 중 제품LOT 이 직접 가리키는 것이 39건이다.
검사기는 `== 0` 만 끊김으로 세고 그래서 정방향·역방향 끊김 **0** 이다.

## ⑥ 다른 게이트 — 하나도 깨지 않았다

| | 회전 시작 | 회전 끝 |
|---|---|---|
| 게이트 요약 | `PASS 19 · FAIL 0 · 차단 13 · 미구현 0 / 32` | **`PASS 20 · FAIL 0 · 차단 12 · 미구현 0 / 32`** |
| pytest | — | **1245 passed · 2 xfailed · 0 failed** |

**움직인 것은 G-08 하나(차단 → PASS)** 다. `G-07`·`G-11`·`KPI`·`G-12`·`G-20`·`G-22`·`G-24`·
`G-26`·`G-27`·`G-28`·`G-29`·`G-30` 전부 PASS 유지. `gate.py` 를 연속 2회 돌려 같은 판정표를 얻었다.
`make db-seed` 를 두 번 돌려 **68표 행 수 diff 0**(G-07 멱등).

- **G-09 는 여전히 `차단`** 이다 — 정확성 100.0% · 연계성 100.0% 는 재졌지만 **정합성 분모가 0** 이다
  (`EST_CAD_FEATURES` 0행 D-05 · `EST_QUOTATIONS`·`EST_COST_RATES` 0행 D-04 · `재질` 코드 0건 D-47).
  분모를 만들려면 재질을 지어내야 하므로 **그대로 두었다.**
- **G-10 은 여전히 `차단`** 이다 — 시계열은 시드가 아니라 수집 산출물이다.
- **`check_ingest ⑤` 를 우회하지 않았다** — `EST_CAD_OBJECTS`·`EST_CAD_FEATURES` 는 0행 그대로다.
- **G-16 은 `차단` 을 유지하되 사유 문장이 바뀌었다**: "EST_BOM_HEADERS 0건" → "BOM 40건 — 기준 BOM
  40건과 대조할 매칭 키가 정본에 없다". 검사기가 스스로 실측을 다시 찍었다(내가 고친 것이 아니다).

## ⑦ 방향을 뒤집은 테스트 (지우지 않았다)

| 파일 | 전 | 후 |
|---|---|---|
| `test_dev1_seed.py` | `품목·재질·고객사…` 7그룹 **0건** | `재질`·`보관위치`·`불량유형`·`클레임유형`·`원가대상` 5그룹만 0건 + **고객사 19종에 역할·`확정 (D-139)`·확신도가 있어야 한다** + **품목에 `가설 (D-131)`·견적 출처가 있어야 한다** + **합성 선언이 DB 에 한 곳만 있어야 한다** |
| `test_arch_seed.py` | `seed.py EMPTY_GROUPS` 전 그룹 0건 | `EMPTY_GROUPS` 에서 고객사·품목을 **`MOVED_TO_DEV1` 로 옮겼다** — 테스트는 그대로 두고 정본 목록이 줄었다 |
| `test_dev1_flow.py` | teardown 이 `품목`·`재질` **0건** | `재질` 0건 + **테스트가 만든 품목 코드만 사라졌는지** + **시드가 넣은 `가설 (D-131)` 품목이 남아 있는지** |
| `test_dev3_seed.py` | `EST_PROJECTS` **0건** | 개발3 시드 소스에 `insert into EST_PROJECTS` 가 없는지 + **채워졌으면 `GD`+YYMM 실측 형식이고 고객사에 `확정 (D-139)` 표시가 있어야 한다** |
| `test_dev3_hitl.py` | `EST_QUOTATIONS`·`EST_BOM_HEADERS` **0건** | `EST_QUOTATIONS` 만 0건 + **BOM 40건 전부에 합성 표시가 붙어 있고 013 화면에 배지가 있어야 한다** |
| `test_qa2_data.py` | `EST_PROJECTS` 0건 · `SHP_LOT_TRACES` 0건 · 정확성 분모 0 · KPI 표본 0 | 0건 경로는 **롤백 트랜잭션에서 표를 비워** 실증(커밋하지 않는다)하고, N건 경로는 **증분**으로 잰다. **새 테스트 2건**: `G-08 판정 줄에 합성 고지·표본 수·임계 85% 가 같이 있는지` · `의도적 단절 표본이 있어서 100% 가 아닌지` |

## ⑧ 회전 중 드러난 결함 2건 (합성 데이터를 넣기 전에는 보이지 않았다)

- **D-210** `POST /shp/016` 출하번호 채번이 `count(*) + 1` 이라 연도별 시드 번호와 충돌해
  `SHIPMENT_NO` UNIQUE 위반 **500**. 그 해의 마지막 일련번호 + 1 로 고쳤다.
- **D-211** `/kpi/044` 클레임 발생률 카드의 보조설명이 `분모 0` 으로 **오탐**됐다
  (분모는 출하 39건, 분자가 0). 검사기를 고치지 않고 카드 보조설명을 `분모 … · 분자 …` 로 갈랐다.
- **D-206-정정** 출하 품목 코드를 `PRODUCT_GROUP`(제품군 코드)에서 가져오던 것이 틀렸다 —
  제품군과 품목은 다른 코드 그룹이다. 시드·라우터 둘 다 **BOM 레벨1 품목**을 쓰도록 고쳤다.

## ⑨ 재현 명령

```
make db-reset && make db-seed          # 멱등 — 두 번 돌려 68표 행 수 diff 0
uv run pytest -q                       # 1245 passed · 0 failed
uv run python tools/check_data.py      # G-08 PASS  합성 데이터 기준 D-131 · 표본 40 · 87.5%
uv run python tools/gate.py            # PASS 20 · FAIL 0 · 차단 12 · 미구현 0 / 32 (2회 연속 동일)
```

**새로 만든 실측 산출물** `docs/cad/thread_projects.json`(프로젝트 12 · 도면 341 · 선정 규칙) ·
`docs/cad/quote_items.json`(품목 179 · 제외 목록 20표기/86행). 둘 다 원본 아카이브를
**읽기만** 했고 원본을 고치지 않았다.
