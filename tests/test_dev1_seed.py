...
def test_D47_정본에_값이_없는_코드그룹은_여전히_비어있다():
    """보관위치·불량유형·클레임유형·원가대상은 화면 입력 마스터다 (D-47).

    **고객사·품목은 여기서 빠졌다** — 사용자가 확정(D-139)·가설(D-131)로 채우라고 지시했다.
    그 둘은 아래 두 테스트가 **표시가 붙었는지**로 단언한다(0건 단언을 지우지 않고 뒤집었다).
    **'재질' 도 같은 이유로 빠졌다** — 가정 답변 ②(D-150)에서 개발3 시드가 채우고,
    `tests/test_dev3_seed.py` 가 **가정 표시가 붙었는지**로 단언한다. 방향을 뒤집은 것이고
    0건 단언을 지운 것이 아니다.
    """
    for group in ("보관위치", "불량유형", "클레임유형", "원가대상"):
...
def test_시드가_공통시드_영역을_다시_넣지_않는다():
    ...
    # '재질' 은 **개발3** 이 가정 답변 ②(D-150)에서 채운다 — 개발1 시드에는 없다(아래 단언).
    assert groups == {"공정", "제품군", "검사구분", "공급구분", "외주구간", "설비",
                      "LOT채번", "고객사", "품목", "재질"}, groups
    assert "insert into BAS_COMMON_CODES" in src, "개발1 은 코드 그룹을 넣는다"
    assert "'재질'," not in src, "'재질' 코드를 넣는 것은 개발3 시드 몫이다 (D-150)"
