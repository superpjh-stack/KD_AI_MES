...
from kyungdong.app.util import assumed, clock            # noqa: E402
...
def rollback_after():
    ...
    #   · '재질' 도 '품목' 과 같아졌다 — 개발3 시드가 답변 ②(D-160)에서 채운다. 그래서
    #     0건 단언을 **방향을 뒤집어** 유지한다: 테스트가 만든 값은 사라졌는지 + 시드가 넣은
    #     선언 파생 행은 남았는지. 시드 행을 지우면 그게 결함이다.
    #     패턴은 `assumed.ATTR1_LIKE` 다 — 문구(`가정 답변 기준` → `도입기업 확정`)를 박아
    #     두면 성격이 바뀌는 순간 이 단언이 0 > 0 으로 깨진다. 실제로 깨졌다.
    ...
    assert int(conn.q1("select count(*) as n from BAS_COMMON_CODES where CODE_GROUP = '재질' "
                       "and ATTR1 like %s", (assumed.ATTR1_LIKE,))["n"]) > 0, \
        "시드가 넣은 선언 파생(D-160) 재질 코드까지 지웠다"
