...
```
QA3 몫 17 게이트:  PASS 8 · FAIL 0 · 차단 7 · 판정 불가 2      (회전 7 재측정)
전체 게이트표:      PASS 19 · FAIL 0 · 차단 13 · 미구현 0 / 32
...
**회전 7 — G-25·G-26·G-27·G-29 의 FAIL 이 해소됐다.** 전부 "데이터 부재와 무관하게 코드 경로가
0곳" 이던 것들이고, 그 코드가 **실제로 붙었는지 먼저 실측한 뒤** 판정을 옮겼다(§0.2).

> **G-25 는 `PASS` 가 아니라 `차단`이다.** 롤백 경로 하나가 열렸을 뿐, 남은 4개(모델 버전·성능·상태 /
> 학습 이력 / 예측 기록 / Train·Val·Test 분리)는 **학습 미실시로 분모 0** 이다. 1/5 충족을 PASS 로
> 올리는 것은 게이트를 낮추는 일이다 — `차단` 은 "아직 못 잰다" 지 "통과했다" 가 아니다.
...
| G-25 MLOps | **차단** (회전 7 · FAIL→차단) | 4/5 미충족이 **전부 분모 0**. 롤백 경로는 **v1.0→v0.9 복귀 실측 성공** |
| G-26 전송·저장 보안 | **PASS** (회전 7) | **CSRF 31/31 + `ENFORCE=1`** · 켠/끈 상태 실패집합 diff 0줄 |
| G-27 계정·잠금 | **PASS** (회전 7) | `POST /login` **200+세션쿠키** · 틀린 비번 **401** · 실패 5회 **403 잠금** · 짧은 비번 **422** |
| G-28 RBAC 6×8 | **PASS** | 270회 실측 불일치 0 · EXEC = 조회/승인(등록·수정 없음) |
| G-29 감사 | **PASS** (회전 7) | 45화면 **전부 기록(미기록 0)** · 거부 18회 → `오류` **18행** · URL 평문 비밀번호 **0곳** |
...
uv run pytest -q tests/test_qa3_ai.py tests/test_qa3_security.py
uv run pytest                            # 1210 passed, 2 xfailed  (failed 0 · skip 0)
uv run python tools/gate.py              # PASS 19 · FAIL 0 · 차단 13 · 미구현 0 / 32  (2회 연속 동일)
```
...
## 0.2 회전 7 — 해소된 결함 (§4.4-2 · 실측 재확인)

**판정을 바꾸기 전에 결함이 실제로 고쳐졌는지 먼저 쟀다.** 명령과 값은 아래 그대로다.
표지 테스트는 **지우지 않고 방향을 뒤집었다** — "아직 없다" 를 "있다, 없어지면 깨진다" 로.
그래야 기능이 다시 빠질 때 빌드가 잡는다.

| 결함 | 상태 | 실측 (명령 → 값) |
|---|---|---|
| **DEF-QA3-002** 로그인·로그아웃·잠금 미연결 (Critical) | **해소** | `src/kyungdong/app/auth.py` 가 생겼다. `POST /login` 맞는 비번 → **200 + `kyungdong_sid` 쿠키 + 서버측 세션 1** · 틀린 비번 → **401** · 실패 `LOGIN_FAIL_MAX=5` 회 뒤 맞는 비번으로도 → **403 잠금** · 실패도 `SYS_ACCESS_LOGS` 에 `접속`/`오류` 로 남는다. 회귀 표지: `test_g27_login_and_lockout_are_wired` |
| **DEF-QA3-003** 비밀번호 정책 미강제 (High) | **해소** | `auth.policy_errors()` 가 강제한다. `'Ab1!'` → `['최소 10자 이상…', '…3종류 이상…']` · `POST /password` 짧은 비번 → **422**. 임계는 여전히 `.env` **가설 (D-16)** 이고 배지로 드러난다 — **QA 가 값을 정하지 않았다**. 회귀 표지: `test_g27_password_policy_is_enforced` |
| **DEF-QA3-004** 45화면 중 16개 접속 감사 미기록 (High) | **해소** | 45화면 전수 GET → **접속 로그 없는 화면 0개** (측정 당시 16개). 원인이던 개발2 16화면이 공용 가드를 지난다. 회귀 표지: `test_g29_every_screen_logs_access` (한 화면이라도 빠지면 깨진다) |
| **DEF-QA3-005** `LOG_TYPE='오류'` 0곳 (High) | **해소** | 권한 없는 (역할,화면) 조합 **18건 전수** GET → 전부 403 · `오류` 감사 **18행**(1:1). 예: `조회거부 / 오류 / '사용자/시스템관리 조회 권한 없음 (역할 QUALITY)'`. 쓰는 곳 `main.py:123`(공용 가드) · `dsh.py:60`. 회귀 표지: `test_g29_error_log_type_is_written` |
| **DEF-QA3-006** 발급 비밀번호가 URL 쿼리스트링으로 (High) | **해소** | `grep "issued=\{?raw" src/` → **0곳** (이전 `routers/sys.py:151`). `/sys/026` 응답에 **Location 리다이렉트가 없다** — `_users_page(issued_password=…)` 로 **POST 본문 응답**이 됐다. 회귀 표지: `test_g29_issued_password_never_goes_through_url` (코드 grep + 응답 헤더 둘 다 본다) |
| **DEF-QA3-009** API 감사가 두 갈래에서 빠진다 (Medium) | **해소** | ① `POST /api/agent/query` → `API` 감사 **1행**(이전 0) ② `/inv/009` **501** 갈래 → `API` 감사 **1행 · `RESULT_CODE='오류'`**(이전 0). 정상 갈래(200)도 1행 유지. **못 한 것을 정상으로 적지 않는다.** 회귀 표지: `test_g29_api_audit_covers_json_api_and_501` |
| **DEF-QA3-007** MLOps 롤백 경로 0곳 (High) | **부분 해소 — 롤백만** | `ml/registry.py` 에 `deploy()`·`rollback()`·`previous_version()` + `POST /est/014/deploy`. v0.9(검증)·v1.0(배포) 두 버전을 넣고 `mlreg.rollback()` → **`v1.0 → v0.9` · 되돌린 뒤 배포중 v0.9 · 직전 배포본 1건 강등 · 이력 2행 그대로 보존**. 되돌릴 곳 없으면 **422**(성공한 척하지 않는다). **남은 `EST_ML_MODELS` 쓰기(등록) 경로 0곳은 미해결** — 학습을 실행하지 않아서다 |

### 해소되지 **않은** 것 — 바꾸지 않은 이유

| 결함 | 왜 그대로 두었나 |
|---|---|
| **DEF-QA3-008** RAG 임계 0.70 (D-10 가설)이 실측 분포와 안 맞는다 | **손대지 않았다.** 임계를 내리면 G-21·G-22 숫자가 좋아 보이지만 그건 게이트를 낮추는 일이다. D-10 은 가설이고 정본에 수치가 없다 — **QA 가 정할 값이 아니다** |
| **G-25 의 나머지 4개** (모델 버전·성능·상태 / 학습 이력 / 예측 기록 / Train·Val·Test 분리) | **분모 0**이다. 학습을 실행하지 않았고 견적 Label 원천이 없다(D-04) · Train/Val/Test 비율이 정본에 없다. 합성 라벨·합성 모델 행으로 채우면 그게 조작이다(§10-18). 그래서 G-25 는 **`차단`** 이고 `PASS` 가 아니다 |
| **DEF-QA3-010** Confidence·근거 동시 표시 1/5 화면 · **DEF-QA3-011** 라벨링 클래스 불일치 · **DEF-QA3-012** `SHP_INSPECTIONS` 쓰기 경로 0곳 | 회전 7 에서 **다시 재지 않았다** — 이번 작업 범위(FAIL 4건 + 표지 11건) 밖이다. 앞선 실측 그대로 미해결로 둔다 |

> **번호 주의.** 이 리포트의 `DEF-QA3-nnn` 과 `tests/test_qa3_security.py` docstring 의 번호가
> 어긋나 있다(리포트 002=로그인 ↔ 테스트 docstring 004=로그인). **이번에 번호를 바꾸지 않았다** —
> 번호를 옮기면 앞선 회전의 인용이 전부 깨진다. 대응은 위 표의 **설명 문구**로 읽는다.

---

## 1. 결함 (DEF-QA3-nnn)

### ~~DEF-QA3-001~~ · **해소** (원 등급 Critical) · CSRF 가 POST 27개 중 0곳에 연결돼 있었다 — 담당 전원
...
### ~~DEF-QA3-002~~ · 로그인·로그아웃·실패 잠금 미연결 → **해소 (회전 7)**

> **회전 7 — 해소.** 실측은 §0.2 표에 있다. 아래는 당시 결함 기록이며 회귀 판단의 기준선으로 남긴다.
...
### ~~DEF-QA3-003~~ · 비밀번호 복잡도·주기 미강제 → **해소 (회전 7)**
...
### ~~DEF-QA3-004~~ · 45화면 중 16개 접속 감사 미기록 → **해소 (회전 7 — 미기록 0개)**
...
### ~~DEF-QA3-005~~ · `LOG_TYPE='오류'` 를 남기는 코드가 0곳 → **해소 (회전 7 — 거부 18회 = 오류 18행)**
...
### ~~DEF-QA3-006~~ · 발급 비밀번호가 URL 쿼리스트링으로 전달된다 → **해소 (회전 7)**
...
### DEF-QA3-007 · **부분 해소** · MLOps — ~~롤백 경로 0곳~~ **해소** · **모델 등록 쓰기 경로 0곳 미해결** — 담당 **개발3**

> **회전 7.** 롤백은 실측으로 동작한다(`v1.0 → v0.9` · 없으면 422 — §0.2). **등록 쓰기 경로는 여전히 0곳**이고
> 그것은 학습 미실시(분모 0)의 결과다. G-25 는 그래서 `FAIL` 이 아니라 **`차단`** 으로 옮겼다 — `PASS` 가 아니다.
...
### ~~DEF-QA3-009~~ · API 감사가 두 갈래에서 빠진다 → **해소 (회전 7 — 두 갈래 모두 기록)**
...
| 담당 | 결함 |
|---|---|
| **전원 (아키텍트 조율)** | ~~DEF-QA3-001 CSRF 0/27~~ → **해소** (31/31 + `ENFORCE=1`) |
| **개발1** | ~~DEF-QA3-002 로그인·잠금~~ · ~~DEF-QA3-003 비밀번호 정책~~ · ~~DEF-QA3-006 URL 평문 비밀번호~~ → **전부 해소** (회전 7 · §0.2) |
| **개발2** | ~~DEF-QA3-004 16화면 접속 감사 미기록~~ → **해소** · DEF-QA3-012 `SHP_INSPECTIONS` 쓰기 경로 부재 **미해결** |
