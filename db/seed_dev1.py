...
NOT_SEEDED: list[tuple[str, str]] = [
    ("INV_MATERIAL_LOTS.MATERIAL · EST_BOM_ITEMS.MATERIAL (값)",
     "재질 **코드 그룹**은 개발3 이 도입기업 답변 ②(D-160)에서 채웠다 — 도면 표제란 실측 14종을 "
     "8종으로 통합한 것이고 `ATTR1` 에 출처 표기가 붙어 있다. 그러나 그것은 **값의 검증표**이고 "
     "합성 사슬의 자재LOT·BOM 항목에 어느 재질이 들어갔는지는 정본에도 답변에도 없다 → "
     "MATERIAL 은 NULL 로 둔다. 그래서 G-09 정합성 'BOM 자재 ↔ 자재LOT 재질' 분모는 여전히 0 이다. "
     "**재질 코드가 서면 G-09 분모가 열린다고 내가 적었던 것은 틀렸다**(D-152) — 실제로 Feature "
     "672행을 넣어도 분모는 0 이었다. 답변 ② 는 이 차단을 풀지 못한다"),
