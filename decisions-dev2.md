...
| **D-208** | 결함 | **`tools/check_routes.py` G-03-③ 이 아키텍트 파일에서 타 사업 용어 1건을 잡는다** — `src/kyungdong/app/util/security.py:3` 의 `'타월'`("직전 사업(송월타월)에서 …") | **개발2 파일이 아니라 고치지 않았다.** 근거 D-번호가 주변 160자 안에 없어서 걸린다(D-37 규칙). 아키텍트가 그 문장에 `(D-02)` 같은 근거를 붙이거나 문구를 바꾸면 G-03 이 PASS 된다. 개발2 소유 파일 11개(`routers/{dsh,prc,shp,kpi}.py` · `app/kpi.py` · `templates/{dsh,prc,shp,kpi}/*` · `board.html` · `db/seed_dev2.py`)는 **0건**이다 |
