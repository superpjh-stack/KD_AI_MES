...
# 회전 — 가정 답변(D-150) 예행: 파이프라인이 도는지 본다

> **⚠ 도입기업 답변은 오지 않았다.** 사용자 지시(2026-09-12 "요청서를 받았다고 가정하고
> 진행해줘")로 가정 답변을 `docs/assumed/customer_answers.json` (D-150) 에 모아 두고,
> **그 파일 안의 값에서만** 파생했다. 이것은 **AI 정확도를 증명하는 작업이 아니다** —
> 데이터가 실제로 도착했을 때 파이프라인이 도는지 보는 **예행**이다.
>
> **G-14 수치는 정확도의 증거가 아니다.** 라벨과 예측을 우리 DXF 파싱에서 **둘 다** 뽑았다.
> 우리 탐지를 우리가 만든 정답과 비교하므로 **순환**이고, 거기서 나온 F1 0.9009 는
> **측정이 아니라 우리가 고른 값**이다.

## ① 가정 선언에서 파생한 것 — 전부

선언은 **두 겹이고 둘 다 있어야 한다**(`src/kyungdong/app/util/assumed.py`).
① 파일 `docs/assumed/customer_answers.json` ② DB `SYS_CONFIGS('시스템설정','ASSUMED_ANSWERS')='D-150'`.
한쪽만 있으면 `declaration()` 이 `None` 이고 파생물은 **전부 차단**이다. G-08 의
`SYNTHETIC_THREAD`(D-131) 와 같은 방식이고, 선언을 지우면 **고지와 파생물이 함께 사라진다.**

| 파생물 | 어디에 | 어느 답변에서 | 선언을 지우면 |
|---|---|---|---|
| 선언 1행 | `SYS_CONFIGS('시스템설정','ASSUMED_ANSWERS')='D-150'` | `_meta.결정번호` | 파일이 없으면 **행을 지운다** |
| 재질 코드 8종 | `BAS_COMMON_CODES` `CODE_GROUP='재질'` | ② `통합_제안` + `docs/cad/dwg_features.json` 실측 | **8행 회수 → 0건** (실측) |
| 단위 가정 표지 | `EST_CAD_FEATURES.SOURCE_DESC` + `UOM` 도면단위→mm | ③ `가정_답변`·`위험` | 표지 안 붙음 · 단위 안 바뀜 · 표지 붙은 행 회수 |
| 정답 라벨 3,391 / 예측 3,391 | `work/cad_labelset.json` | ④ `가정_방식` | 생성기가 **거부(exit 1)** · G-14 가 다시 `차단` |
| 고지 문구 4종 | `assumed.base_note/circular_note/unit_tag/material_attr1` | `_meta.경고` · ④ `정직한_판정` · ③ `위험` | **빈 문자열** |

**고지 문구를 코드에 박지 않았다.** 검사기·시드·화면이 쓰는 문구는 전부 선언 파일의
백틱 안에서 꺼낸다(`_QUOTED`). 문구를 박아 두면 선언을 지워도 문구가 남아 거짓 증거가 된다.

**`제공되지_않은_것` 5건은 채우지 않았다** — G-15·G-17·G-18 은 **차단이 정답**이고 그대로 차단이다.

## ② 재질 코드 (통합 전 14종 → 통합 후 8종) · 단위 가정 Feature

출처는 `docs/cad/dwg_features.json` `요약.실측 합계.재질 분포` — **도면 586건 실측**이다.
**실측 목록 밖의 재질은 만들지 않았다**(`tests/test_dev3_seed.py::test_재질_코드는_실측_목록_밖의_이름을_만들지_않는다`
가 목록 밖 이름이 하나라도 있으면 FAIL 한다).

| 코드 | 실측 도면 | 통합된 표기 |
|---|---|---|
| `STS304` | 282 | STS304 200 · SUS304 69 · STS304F 12 · SUS304L 1 |
| `STS316L` | 234 | STS316L 199 · SUS316L 28 · A240316L 7 |
| `SS400` | 52 | SS400 52 |
| `STS316` | 9 | STS316 6 · SUS316 3 |
| `A516` | 6 | A516 6 |
| `AL6061` · `ALUMINUM` · `STS430` | 각 1 | 그대로 |

`ATTR1` = `가정 답변 기준 (D-150) — 도입기업 미확인` · `ATTR2` = `실측 도면 N건 — <원표기 분포>`.
**두께는 코드 그룹으로 만들지 않았다** — 수치이고 `EST_CAD_FEATURES` 에 있다. 코드로 굳히면
새 두께가 422 가 된다. (관찰: 가정 답변 ② 는 "두께 15종" 이라 적었으나 **실측 distinct 는 43종**이고
요약 JSON 의 15종은 상위 15종 절단이다. 어느 쪽도 코드로 만들지 않았다.)

### 단위 가정 — 620건을 mm 로 본다, 단 표지를 남긴다

`EST_CAD_DRAWINGS` 의 DXF·DWG **190건**에 적재를 돌린 실측(`tools/cad_ingest.py --dxf-features --dxf-dwg`):

| | 건수 |
|---|---|
| 대상 도면 | 190 (DWG 183 + DXF 7) |
| 읽힌 도면 | 179 · 변환/원본 실패 **11** |
| 적재된 Feature | **672 행** |
| **단위 미상 도면** | **122 건** → 그 Feature **508 행**에 표지 |
| 단위가 이미 명시된 도면 | **57 건 (mm 52 · inch 5) — 건드리지 않았다** |

`SOURCE_DESC` 실측 1행:
```
DXF 실측(dwg2dxf 변환) · 단위 가정 (D-150) 도면단위→mm · ⚠ inch 도면을 mm 로 읽으면 25.4배 오차다 — GD170402-01-콘베어-L.dwg
```
표지는 `SOURCE_DESC varchar(300)` 안에서 **절대 잘리지 않게 앞에 둔다** — 본문만 자른다.
`UOM` 은 `도면단위 → mm` · `도면단위² → mm²` 로 바뀌고, **표지 없이 단위만 바뀌는 경로는 없다**
(`tests/test_dev3_cad.py::test_단위_가정은_표지를_달고_붙고_명시된_단위는_건드리지_않는다`).

**기본 상태의 `EST_CAD_FEATURES` 는 0행이다 — 적재는 옵션이다.** 이유는 ④ 에 적었다.

## ③ 라벨 30건 — FN·FP 주입 비율과 위치

`tools/gen_cad_labelset.py` → `work/cad_labelset.json`. **어휘는 TD5 `EST_CAD_OBJECTS.OBJECT_TYPE`
5종을 썼다** (홀·슬롯·노즐·플랜지·치수문자). `docs/cad/라벨링 가이드.docx` 의 3종
(`title_block`·`bom_table`·`rev_table`)은 **쓰지 않았다** — TD5 에 담을 자리가 없다(D-98).
5종 중 실제로 라벨이 생긴 것은 **홀뿐**이고, 나머지 **4종은 0건**이다: 우리 파서는 CIRCLE 을 셀 뿐
슬롯·노즐·플랜지·치수문자를 가릴 기준이 없다(D-05). **빈 4종을 박스로 채우지 않았다.**

| | 값 |
|---|---|
| 도면 | **30 건** (DWG 29 · DXF 1 / 단위 도면단위 27 · mm 2 · inch 1) |
| 건너뛴 도면 | 7 (변환 실패 3 · CIRCLE 0개 4) — 사유를 `_meta.읽지_못한_도면` 에 전부 적었다 |
| 파싱 전량 박스 | **3,768** (CIRCLE 외접 정사각형 `[cx±r, cy±r]` — 좌표는 도면 좌표 그대로) |
| `labels` | 3,391 | 
| `predictions` | 3,391 |

**주입 — 무작위 없음. 재실행하면 같은 자리다.**
도면을 `DRAWING_ID` 순으로, 도면 안은 DXF 읽은 순서로 돌며 **전체 통번호 `i`(0부터)** 를 붙이고
`i` 로만 자리를 정한다.

| 주입 | 규칙 | 비율 | 건수 | 위치(통번호) |
|---|---|---|---|---|
| **FP(오검출)** | `i % 10 == 3` → **라벨에서 뺀다** | **10%** | **377** | 3, 13, 23, 33, 43, … 3763 |
| **FN(누락)** | `i % 10 == 7` → **예측에서 뺀다** | **10%** | **377** | 7, 17, 27, 37, 47, … 3767 |

도면별 분포(상위): `DWG-RTK-0013` FP 144·FN 144 · `DWG-RTK-0017` 44·44 · `DWG-RTK-0043` 27·28 ·
`DWG-VDR-0038` 26·25 · `DWG-VDR-0040` 25·26. 주입이 **한 건도 없는 도면 6건**
(`DWG-RTK-0002`·`0003`·`0026` · `DWG-VDR-0002`·`0024`·`0029`) — 박스가 10개 미만이라 슬롯에 안 걸린다.
**377개 자리 전부가 `_meta.주입.FP 자리`·`FN 자리` 에 통번호·도면·박스로 적혀 있다.**

**예측도 실제 탐지기의 출력이 아니다.** `cad.provider` 는 Parsing·Vision 둘 다 501 미구성(D-05)이라
탐지 결과가 0건이다. `predictions` 는 DXF 기하 파싱에서 FN 주입분을 뺀 것이고, `score 1.0` 은
확신도가 아니라 자리채움이다. 그 사실을 `_meta.예측의_정체` 에 적었다.

**주입한 만큼 세어지지 않았다 — 그게 실측이다.**
기대 TP/FP/FN = 3014/377/377 ↔ **실측 3055/336/336**, 차이 **41건**.
같은 도면 안에 **좌표가 겹치는 CIRCLE**(라벨 중 중복 좌표 34개)이 있어 뺀 자리를 다른 박스가
대신 매칭한다 — 탐욕 매칭(정답 1개 ↔ 예측 1개)의 성질이다. **맞추려고 고치지 않았다.**
검사기가 이 차이를 그대로 찍는다.

**⚠ 이 라벨셋은 IoU 임계에 둔감하다.** 짝이 남은 라벨·예측은 **같은 박스**라 IoU = 1.0 이다.
임계 민감도(되돌림 시험)는 여전히 `evaluator_selftest`(부분 겹침 박스)만이 증명한다 —
박스를 흔들어 IoU 를 만들어 내지 않았다. 그건 좌표를 지어내는 것이다.

### `EST_CAD_OBJECTS` 적재는 `check_ingest ⑤` 와 충돌한다 — **우회하지 않았다**

`tools/check_ingest.py` ⑤ 는 "확정(`CONFIRM_YN='Y'`) 객체 0건 + `EST_CAD_FEATURES` N행 = 합성"
으로 판정한다. **실측으로 확인했다** — Feature 672행을 적재하고 잰 결과:

```
G-13   FAIL   … 결함 2건 …
  결함 G-13 확정 객체가 0건인데 EST_CAD_FEATURES 가 672행이다 — 원천 없이 만들어진 합성 Feature 다 (§10-18)
  결함 G-13 차단된 Feature 가 적재돼 있다: ['두께', '판재 면적', '재질', '총 절단장'] — 원천(D-05)이 없는데 값이 있다면 지어낸 것이다 (§10-18)
```

**G-13 이 `차단` → `FAIL` 로 떨어진다.** 그래서 **적재를 옵션으로 두고 되돌렸다** —
`store_dxf_features(allow_unconfirmed=True)` 는 명시 요구가 있어야 쓰고,
`tools/cad_ingest.py --dxf-features` 없이는 한 행도 쓰지 않는다. 기본 상태는 0행이고 G-13 은 `차단`이다.
**검사기의 판정 로직·임계값은 한 글자도 고치지 않았다.**

**HITL 승인 경로가 맞는지도 실측했다**(`tests/test_dev3_cad.py::test_HITL_승인이_확정을_만들고_그_확정으로만_Feature_가_생긴다`):
① 객체를 넣어도 승인 전에는 Feature 0건 → ② `OPERATOR` 승인은 **403**, `CONFIRM_YN` 은 `'N'` 그대로 →
③ `SYSADMIN` 승인 → `CONFIRM_YN='Y'` → ④ `build_features()` 가 `홀 수량` 1행을 만든다.
**`review_object()` 가 확정을 만드는 유일한 경로라는 것이 맞다.** 넣은 행은 전부 지웠다
(`EST_OBJECT_REVIEWS` 0 · `EST_CAD_OBJECTS` 0 — G-11 을 오염시키지 않는다).
즉 규칙이 요구하는 경로는 살아 있고, 막힌 것은 **1단계(인식)가 501** 이라 객체가 생기지 않는 것뿐이다.

## ④ 게이트 판정과 고지 문구 원문

**판정은 하나도 바뀌지 않았다** — `PASS 19 · 차단 12` 로 회전 전과 같다(2회 연속 동일).
바뀐 것은 **판정 줄에 실린 실측과 고지**다.

### G-14 — `판정 불가` (gate.py 표에서는 `차단`으로 접힌다)
```
G-14 판정 불가 — CAD 객체 인식 80% 이상 (IoU 0.5 · P/R/F1) / 가정 답변 기준 (D-150) · 순환 라벨 — 정확도 증거 아님 · IoU 0.5 TP 3055 FP 336 FN 336 P 0.9009 R 0.9009 F1 0.9009 · 도면 30건 · 라벨·예측을 같은 DXF 파싱에서 파생(순환) — 정확도 증거 아님. 실제 탐지기는 여전히 501 미구성(D-05)이고 EST_CAD_OBJECTS 0행이다
```
**고지가 수치 앞에 있다.** `tools/gate.py` 가 상세를 120자로 자르므로, 잘려도 고지가 먼저 남고
숫자만 떼어 인용될 수 없다. `PASS` 로 올리지 않았다 — 순환 라벨로는 80% 를 넘든 못 넘든
**게이트를 판정할 수 없다.** 그래서 `판정 불가`다. 임계값(80%)은 건드리지 않았다.

### G-19 — `차단`
```
G-19 차단 — 가정 답변 기준 (D-150) · 순환 라벨 — 정확도 증거 아님 · 3단계 Feature 0행(단위 가정 표지 0행) · 5단계 종단 통과 0건 — 1단계(인식)가 501 CAD Parsing 미구성(D-05)에서 멈춘다. 확정객체 0 → 견적 0 → SHAP 0. 가정 답변은 Autodesk API·YOLOv8 가중치를 주지 않으므로 1단계는 **가정으로 열리지 않는다**. 화면 Confidence·근거 표기 1/5 화면
```
**G-19 는 가정으로 열리지 않는다** — 막는 것이 도입기업 답변이 아니라 D-05(Autodesk API·YOLOv8
가중치 미확보)이기 때문이다. 가정 답변에 그것을 넣을 자리가 없고, 넣지 않았다.

### G-09 — `차단` (움직이지 않았다 · **가정 답변 ② 의 주장이 실측과 다르다**)
```
G-09 차단 — 분모 0 → 판정 불가(규약 §5): 정합성 (측정된 것: 정확성 100.0 % · 정합성 판정 불가(분모 0) · 연계성 100.0 %) · §4-② NOT NULL 위반 0 · §4-③ FK 고아 0/98제약 · §4-④ 판정 제외
  가정 파생값이 G-09 수치에 들어온 행 0 — 없음
     선언은 있으나 **판정에 섞이지 않았다**: '재질' 코드 그룹이 서도 `INV_MATERIAL_LOTS.MATERIAL`·`EST_BOM_ITEMS.MATERIAL` 과 `THICKNESS_MM`·`LENGTH_MM` 이 전부 NULL 이라 정합성 분모가 열리지 않는다. 가정 답변 ② 가 적은 '정합성 분모가 열린다' 는 **실측과 다르다** — 코드 그룹은 값의 검증표일 뿐 값을 만들지 않는다
```
Feature 672행을 적재한 상태에서도 정합성 5항목의 분모가 **전부 0** 이었다(실측). 고지는
**실제로 섞였을 때만** 붙는다(`assumed_used()` 가 행 수를 센다) — 안 섞였는데 붙는 고지는 노이즈고,
섞였는데 없는 고지가 거짓 증거다.

### G-13 — `차단` 유지 (적재를 되돌렸으므로)
```
G-13 차단 — … 결함 0건. 남은 미충족은 원천 부재뿐 — Feature 5/6종이 D-05 … 로 차단
```

**G-15·G-17·G-18 은 손대지 않았다** — `제공되지_않은_것` 이라 **차단이 정답**이다. 전부 `차단` 유지.

## ⑤ 선언을 지웠을 때 차단으로 돌아가는가 — **직접 시험했다**

### 시험 A — DB 선언만 지운다 (`delete from SYS_CONFIGS … 'ASSUMED_ANSWERS'`)
```
G-14 차단 — … / 분모 0 — … · work/cad_labelset.json 에 라벨 3391건이 있으나 가정 선언이 없다
             (파일은 D-150 인데 SYS_CONFIGS('시스템설정','ASSUMED_ANSWERS') 선언이 없다 — `make db-seed` 를 돌리지 않았다)
G-19 차단 — 5단계 종단 통과 0건 — …   ← **고지가 사라졌다**
```
라벨 파일이 그대로 있어도 **수치를 내지 않는다.** 출처를 확인할 수 없는 라벨로 수치를 내면
그것이 곧 거짓 증거이기 때문이다.

### 시험 B — 출처 파일을 치우고 `make db-seed`
```
가정 선언        SYS_CONFIGS('시스템설정','ASSUMED_ANSWERS') = 없음 — 가정 선언 없음 — 파일 docs/assumed/customer_answers.json 도 … 도 없다
재질 코드        실측 표기 14 종 → 통합 후 0 종 (회수 8 행) · 표시 없음
   psql: 재질 코드 0 · 선언 0
라벨 생성기: 가정 선언이 없다 — 라벨을 만들지 않는다. (exit 1)
           선언 없이 만든 라벨은 가정이 아니라 조작이다.
G-09: 가정 파생값이 G-09 수치에 들어온 행 0 — 없음   ← 고지 없음
```
**파생물이 회수되고 게이트가 전부 차단으로 돌아간다.** 파일을 되돌리고 `make db-seed` 를
다시 돌리면 8종·선언이 그대로 복원된다(실측 확인).
이 되돌림은 테스트로도 고정했다 — `tests/test_dev3_seed.py::test_선언을_지우면_재질_코드가_회수된다`.

## 예행이 잡아낸 것 — 이 작업의 실제 값

1. **`tools/cad_ingest.py --dxf-features` 가 죽어 있었다.** `store_dxf()` 가
   `got["skipped_text"]` 를 읽는데 `pipeline.store_dxf_features()` 는 `text_rows` 를 돌려준다 —
   **첫 도면에서 `KeyError`**. 이 경로는 한 번도 실행된 적이 없었다는 뜻이다. 고쳤다.
   (G-08 합성이 출하 채번 500 · G-11 오탐 · 출하 품목코드를 잡아낸 것과 같은 자리.)
2. **탐욕 매칭이 중복 좌표를 흡수한다** — 주입 377/377 ↔ 실측 336/336. 채점기가 실제로
   세고 있다는 증거이자, 도면에 같은 자리 CIRCLE 이 겹쳐 있다는 실측이다.
3. **가정 답변 ② 의 "G-09 정합성 분모가 열린다" 는 틀렸다.** 코드 그룹은 값의 검증표일 뿐이고
   `MATERIAL`·`THICKNESS_MM`·`LENGTH_MM` 이 NULL 인 한 분모는 0 이다 — Feature 672행으로도 열리지 않았다.
4. **가정 답변 ② 의 "두께 15종" 은 요약 상위 15종 절단**이고 실측 distinct 는 43종이다.
5. **`check_ingest ⑤` 와 Feature 적재의 충돌은 실재한다**(G-13 차단→FAIL·결함 2건).
   우회하지 않고 옵션으로 두었다. 푸는 길은 **1단계 인식이 501 을 벗어나는 것**뿐이고,
   그건 가정 답변이 줄 수 있는 것이 아니다(D-05).
6. **`.cad` 325건은 손대지 않았다** — 가정 답변 ① 이 "재저장 대기로 표시하고 표본에 넣지 않는다"
   고 했고, 그대로 했다. 숫자를 늘리지 않았다.

## 0건 단언 테스트 — 지우지 않고 방향을 뒤집었다

선례(CSRF 표지 3건 · G-08 시드 표지 · 품목/고객사 코드)와 같은 방식이다.

| 테스트 | 전 | 후 |
|---|---|---|
| `test_qa3_ai.py::test_g14_labelset_is_empty_and_says_why` | `labels == []` | `test_g14_labelset_is_assumed_and_says_so` — **표지·순환 고지·"501" 사실이 있는지**. 추가로 어휘 5종 밖 이름 0 · 주입 자리가 고정 규칙인지 · `image` 로 도면이 갈렸는지 |
| `test_dev1_seed.py::test_D47_…비어있다` | `재질` 포함 5그룹 0건 | `재질` 을 뺀 4그룹 0건 유지 + `재질` 은 `test_dev3_seed.py` 가 **가정 표시**로 단언 |
| `test_dev1_seed.py::test_시드가_공통시드_영역을…` | 코드 그룹 9종 | 10종(`재질` 추가) + **개발1 시드가 `재질` 을 넣지 않는지** |
| `test_dev1_flow.py::rollback_after` | `재질` 0건 | 테스트가 만든 재질 코드는 사라지고 **시드의 가정(D-150) 행은 남았는지** |
| `db/seed.py::EMPTY_GROUPS` | `재질` 포함 | `MOVED_TO_DEV3` 로 옮김(선례 `MOVED_TO_DEV1`) — 0건 단언 대상이 아니라는 근거를 함께 적었다 |

## 검증 (보고 전에 직접 실행)

```
make db-reset && make db-seed                 # 재질 8종 · 선언 D-150
uv run pytest -q                              # **1255 passed · 2 xfailed · 실패 0** (직전 1245 → +10, 회귀 0)
uv run python tools/check_ai.py               # G-14 판정 불가 + 고지 + 수치 · G-19 차단 + 고지
uv run python tools/check_data.py             # G-09 차단 (가정 파생값 0행 — 고지 안 붙음)
uv run python tools/check_ingest.py           # G-13 차단 (Feature 0행)
uv run python tools/gate.py                   # PASS 19 · 차단 12
uv run python tools/gate.py                   # **2회 연속 같은 판정 — 차이 없음**
# 선언 제거 시험 (A: SYS_CONFIGS 행 삭제 / B: 출처 파일 이동 + db-seed) → 전부 차단 복귀 확인
```

**회전 전 `PASS 19 · 차단 12` ↔ 회전 후 `PASS 19 · 차단 12` — 판정 변화 0.**
G-07 · G-08 · G-11 · G-12 · KPI · G-20 · G-22 · G-24 · G-26~G-30 전부 PASS 유지.
임계값(80% · ±5% · 85% · 90%)은 한 글자도 건드리지 않았고, `tools/check_*` 에는 **고지만 붙였다**.
