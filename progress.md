...
| 조건 | 실측 | 판정 |
|---|---|---|
| ① 게이트 전건 PASS (`차단`은 사유·D-번호 기재) | **PASS 19 · FAIL 0 · 차단 13 · 미구현 0 / 32**. 차단 사유 D-04·05·09·12·13·47·56 | **만족** |
...
| **D-98** | **라벨링 가이드 클래스(3종)가 TD5 `OBJECT_TYPE`(5종)과 불일치**. 모집단도 835 vs 1,283 |
...
1. **도입기업 자료 요청** (위 표) — 이것 없이는 차단 13건이 풀리지 않는다
